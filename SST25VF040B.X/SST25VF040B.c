...
void WREN_Check();
void WREN_AAI_Check();
void Send_Byte_Array( unsigned int add, unsigned char data[12]);

void Verify(unsigned char byte, unsigned char cor_byte);

unsigned char upper_128[No_of_bytes] = {0};       /* global array to store read data */
                                    /* to upper RAM area from 80H - FFH */

void Portassign()
{
    // configure SDI/SDO pins
    PPSInput(2, SDI1, RPC4);                     // SDI1: RPC4 - In
    PPSOutput(4, RPD0, SDO1);                    // SDO1: RPD0 - Out
    PPSOutput(3, RPB2, SS1);                     // SS1: RPB2 -Out

    PORTSetPinsDigitalIn(IOPORT_C, BIT_4);       // SDI1 - In
    PORTSetPinsDigitalOut(IOPORT_D, BIT_0);      // SDO1 - Out
    PORTSetPinsDigitalOut(IOPORT_B, BIT_2);      // SS1 - Out
    PORTSetPinsDigitalOut(IOPORT_D, BIT_10);      // SCK1 - Out

    #define SCK PORTDbits.RD10
    #define SI PORTDbits.RD0
    #define SO PORTCbits.RC4
    #define CE PORTBbits.RB2
...
/* Input:	ID_addr							*/
/*									*/
/* Returns:	byte:	ID1(Manufacture's ID = BFh or Device ID = 8Dh)	*/
/*									*/
/************************************************************************/
unsigned char Read_ID(UINT8 ID_addr)
{
...
/* Input:	None							*/
/*									*/
/* Returns:	IDs_Read:ID1(Manufacture's ID = BFh, Memory Type (25h), */
/*		 and Device ID (8Dh)					*/
/*									*/
/************************************************************************/
unsigned long Jedec_ID_Read()
...
/*	 AAI mode.							*/
/*									*/
/* Input:	byte1:		1st byte to be programmed		*/
...
int main()
{
    ...
    unsigned char Byte[No_of_bytes] = {"The Read Status Register (RDSR) instruction allows the Status Register to be read. "
            "The Status Register may be read at any time, even while a Program, Erase or Write Status Register cycle is in "
            "progress. When one of these cycles is in progress, it is recommended to check the Write In Progress (WIP) bit "
...
//    }
    ...
    Send_Byte_Array(0X025648, Byte);

    Read_Cont(0X025648, No_of_bytes);

    return 0;
}

void Send_Byte_Array( unsigned int add, unsigned char data[No_of_bytes])
{
    int i;
    for(i=0; i<No_of_bytes; i++)
    {
        Byte_Program(add, data[i]);
        add = add + 1;
    }
}
